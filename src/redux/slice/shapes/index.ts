...
export interface GeneratedUIShape extends BaseShape {
  ...
  streamedBytes?: number; // bytes received so far
  errorMessage?: string;
}
...
const makeGeneratedUI = (p: {
  ...
  generationStage: p.generationStage ?? 'generating',
  generationProgress: p.generationProgress ?? 0,
  streamedBytes: 0,
...
const shapesSlice = createSlice({
  ...
  reducers: {
    ...
    updateShape(
      state,
      // Use a wider patch type so shape-specific fields (generationStage, etc.)
      // can be applied without hitting discriminated-union Partial<Shape> errors.
      action: PayloadAction<{ id: string; patch: Partial<Shape> | Record<string, unknown> }>
    ) {
      const { id, patch } = action.payload;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      shapesAdapter.updateOne(state.shapes, { id, changes: patch as any });
    },
